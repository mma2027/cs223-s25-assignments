// Author : Maxfield Ma
// Date : 4/18/2025
// Description : Using mutex and cond to seamlessly operate a bridge
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
void *Move(void *args){
    ...
    for (int i = 0; i < data->num_trips; i++) {
        ...
        //if someone is on the bridge and they are going the wrong way, just wait
        while ( bridge.num_on_bridge[0] + bridge.num_on_bridge[1] > 0 && bridge.direction != destination){
            pthread_cond_wait(&bridge.free, &bridge.lock);
        }
        //otherwise, set new direction and begin to cross
        ...
        //prearing to arrive
        pthread_mutex_lock(&bridge.lock);
        //person gets off bridge
        bridge.num_on_bridge[destination]--;
        //if they are the last person going this way, broad cast that the bridge is now empty
